...
.section-header { font-size: 12px; font-weight: 700; letter-spacing: 0.14em;
...
def plot_cluster_breakdown(cluster_df, countries):
    ...
    for cluster in sorted_clusters:
        ...
        for country in countries:
            row = cluster_df[(cluster_df["country"] == country) &
...
def plot_activity_mix(act_df, countries):
    n       = len(countries)
    spacing = max(0.12, 0.35 / n)
    titles  = [c[:35] for c in countries] if n > 1 else None
    fig     = make_subplots(rows=1, cols=n, subplot_titles=titles,
                            horizontal_spacing=spacing)
    for j, country in enumerate(countries):
        c_df  = act_df[act_df["country"] == country].sort_values("pct")
        color = ACCENT_COLORS[j % len(ACCENT_COLORS)]
        fig.add_trace(go.Bar(
            x=c_df["pct"], y=c_df["activity"], orientation="h",
            marker_color=color,
            text=[f"{p}%" for p in c_df["pct"]], textposition="outside",
            textfont=dict(color="#ffffff", size=10),
            hovertemplate="<b>%{y}</b><br>%{x:.1f}%<extra></extra>",
            showlegend=False,
        ), row=1, col=j + 1)
        x_max = c_df["pct"].max() * 1.3 if not c_df.empty else 10
    ...
    max_bars = max(len(act_df[act_df["country"] == c]) for c in countries)
    fig.update_layout(
        paper_bgcolor="#0d0d0d", plot_bgcolor="#0d0d0d",
        font=dict(color="#ffffff", family="monospace", size=11),
...
def plot_sankey(seq_df, country):
    c_seq = seq_df[seq_df["country"] == country]
    if c_seq.empty: return None
    pairs_1_2, pairs_2_3 = [], []
    for dev_id, grp in c_seq.groupby("developer_id"):
...
def plot_top_products(sdk_df, countries, show_titles=True):
    ...
    for j, country in enumerate(countries):
        ...
        fig.update_xaxes(gridcolor="#2a2a2a", zeroline=False,
                         tickfont=dict(color="#ffffff"), range=[0, x_max],
                         row=1, col=j + 1)
        fig.update_yaxes(showline=False, tickfont=dict(color="#ffffff"),
                         automargin=True, row=1, col=j + 1)
    max_bars = max(min(sdk_df[sdk_df["country"] == c]["productname"].nunique(), 5)
                   for c in countries)
...
def plot_top_sdks(sdk_df, countries, show_titles=True):
    ...
    for j, country in enumerate(countries):
        ...
        fig.update_yaxes(showline=False, tickfont=dict(color="#ffffff"),
                         automargin=True, row=1, col=j + 1)
    max_bars = max(min(sdk_df[sdk_df["country"] == c]["sdk_name"].nunique(), 5)
                   for c in countries)
    fig.update_layout(paper_bgcolor="#0d0d0d", plot_bgcolor="#0d0d0d",
                      font=dict(color="#ffffff", family="monospace", size=11),
                      height=max(250, max_bars * 55), margin=dict(l=20, r=20, t=40, b=20),
...
def plot_download_comparison(sdk_df, countries):
    totals = sorted({c: round(sdk_df[sdk_df["country"] == c]["downloads_per_100k"].sum(), 1)
                     for c in countries}.items(), key=lambda x: x[1])
    ...
    fig = go.Figure(go.Bar(
        ...
        hovertemplate="<b>%{y}</b><br>%{x:,.1f} per 100K<extra></extra>",
    ))
    fig.update_layout(paper_bgcolor="#0d0d0d", plot_bgcolor="#0d0d0d",
                      font=dict(color="#ffffff", family="monospace", size=11),
                      xaxis=dict(gridcolor="#2a2a2a", zeroline=False,
                                 tickfont=dict(color="#ffffff"), range=[0, x_max]),
                      yaxis=dict(showline=False, tickfont=dict(color="#ffffff"),
                                 automargin=True),
...
def plot_time_series(sdk_df, countries):
    ...
    fig.update_layout(paper_bgcolor="#0d0d0d", plot_bgcolor="#0d0d0d",
                      ...
                      xaxis=dict(gridcolor="#2a2a2a", zeroline=False,
                                 tickfont=dict(color="#ffffff")),
                      yaxis=dict(title="Downloads per 100K", gridcolor="#2a2a2a",
...
try:
    with st.spinner("Loading countries..."):
        kpis_df    = load_geo_kpis()
        act_df     = load_geo_activity_mix()
        cluster_df = load_geo_cluster_breakdown()
        seq_df     = load_geo_sequences()

    all_countries = kpis_df.sort_values("total_developers", ascending=False)[
        "country"].tolist()
    ...
    map_event = st.plotly_chart(
        plot_world_map(all_countries,
                       list(dict.fromkeys(sidebar_countries + st.session_state.map_selected))),
    ...
    if not selected_countries:
        ...
        if mode == "Single Country":
            ...
            row     = kpis_df[kpis_df["country"] == country].iloc[0]
            st.markdown(f"""
            ...
            st.markdown('<div class="section-header">Comparing Countries</div>',
                        unsafe_allow_html=True)
        ...
        with tab_dev:
            if mode == "Single Country":
                ...
                for c in selected_countries:
                    r = kpis_df[kpis_df["country"] == c].iloc[0]
                    rows.append({
                        "Country":        c,
            ...
            st.markdown('<div class="section-header">Activity Mix</div>',
                        unsafe_allow_html=True)
            act_view = act_df[act_df["country"].isin(selected_countries)]
            st.plotly_chart(plot_activity_mix(act_view, selected_countries),
                            use_container_width=True)
        ...
        with tab_sdk:
            try:
                with st.spinner("Loading SDK data..."):
                    ...
                    sdk     = pd.concat(sdk_dfs, ignore_index=True) if len(sdk_dfs) > 1 \
                              else sdk_dfs[0]
                    sdk_countries = [c for c in selected_countries
                                     if c in sdk["country"].values]
                ...
                if not sdk_countries:
                    ...
                    if mode == "Single Country":
                        ...
                        st.dataframe(pd.DataFrame(rows).set_index("Country"),
                                     use_container_width=True)
                        st.markdown("<br>", unsafe_allow_html=True)
                        st.markdown('<div class="section-header">Downloads per 100K by Country</div>',
                                    unsafe_allow_html=True)
                        st.plotly_chart(plot_download_comparison(sdk, sdk_countries),
                                        use_container_width=True)
                        st.markdown("<br>", unsafe_allow_html=True)

                    st.markdown('<div class="section-header">Downloads per 100K Over Time</div>',
                                unsafe_allow_html=True)
                    st.plotly_chart(plot_time_series(sdk, sdk_countries),
                                    use_container_width=True)
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.markdown('<div class="section-header">Top 5 Products</div>',
                                unsafe_allow_html=True)
                    st.plotly_chart(plot_top_products(sdk, sdk_countries,
                                    show_titles=(mode != "Single Country")),
                                    use_container_width=True)
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.markdown('<div class="section-header">Top 5 SDKs</div>',
                                unsafe_allow_html=True)
